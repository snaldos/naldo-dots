...
INCLUDE_RE = re.compile(
    r'^include(?:\s+optional=(?P<optional>true|false))?'
    r'\s+"(?P<path>(?:\\.|[^"\\])*)"\s*;?(?:\s*//.*)?$'
)
...
def decode_quoted_string(encoded: str) -> str:
    try:
...
def expanded_config_lines(path: Path, ancestors: tuple[Path, ...] = ()) -> list[str]:
    """Read a Niri config with top-level includes expanded positionally."""
    path = path.expanduser()
    canonical_path = path.resolve()
    if canonical_path in ancestors:
        chain = " -> ".join(str(item) for item in (*ancestors, canonical_path))
        raise RuntimeError(f"cyclic Niri config include: {chain}")
    ...
    expanded: list[str] = []
    next_ancestors = (*ancestors, canonical_path)
    for line in lines:
        ...
        included_path = Path(decode_quoted_string(match.group("path"))).expanduser()
        if not included_path.is_absolute():
            included_path = path.parent / included_path
        if match.group("optional") == "true" and not included_path.exists():
            continue
        expanded.extend(expanded_config_lines(included_path, next_ancestors))
...
def parse_bindings(path: Path) -> list[tuple[str, str]]:
    lines = expanded_config_lines(path)
    ...
    while line_index < len(lines):
        ...
        if title_match is not None:
            description = decode_quoted_string(title_match.group("title"))
        else:
