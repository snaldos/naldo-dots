...
// ------------------------------------------------------------------- image --
// Estimate the ordinary terminal background alpha around a pixel.
//
// Text, cursor, and highlighted cells can have greater alpha than the normal
// transparent terminal background. Taking the minimum nearby value gives the
// black-hole shadow a clean background alpha rather than a text-shaped mask.
float localBackgroundAlpha(vec2 uv) {
    vec2 px = 1.0 / iResolution.xy;

    float a = texture(iChannel0, uv).a;

    a = min(a, texture(iChannel0, uv + vec2( px.x, 0.0)).a);
    a = min(a, texture(iChannel0, uv + vec2(-px.x, 0.0)).a);
    a = min(a, texture(iChannel0, uv + vec2(0.0,  px.y)).a);
    a = min(a, texture(iChannel0, uv + vec2(0.0, -px.y)).a);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec4 terminalBase = texture(iChannel0, uv);
    float terminalBackgroundAlpha = localBackgroundAlpha(uv);
    ...
    if (b >= bmax) {
        ...
        // Mild chromatic aberration for RGB.
        ...
        for (int i = 0; i < 3; i++) {
            ...
            vec2 suv = mirrorUV(
                center + sp / vec2(aspect, 1.0)
            );

            termRgb[i] = texture(iChannel0, suv)[i];
        }

        // Alpha must follow the warped terminal coordinate too.
        //
        // Use the central, non-chromatically-separated coordinate because alpha is a
        // coverage channel, not a color channel.
        vec2 alphaSp = p - dir * defl;
        vec2 alphaUv = mirrorUV(
            center + alphaSp / vec2(aspect, 1.0)
        );

        float termAlpha = texture(iChannel0, alphaUv).a;
        ...
        fragColor = vec4(
            termRgb + stars(d) * L.star * window * shield,
            termAlpha
        );
    ...
    // Captured rays should use the ordinary transparent background alpha rather
    // than retaining the alpha shape of text or the current-line highlight.
    float bgAlpha = terminalBackgroundAlpha;
    if (!captured) {
        vec3 d = normalize(v);
        ...
        if (d.z < -0.05) {
            ...
            vec4 warpedTerminal = texture(iChannel0, suv);

            bg += warpedTerminal.rgb * toward;

            // Warp terminal alpha along with terminal RGB. As the ray turns away from the
            // terminal plane, fall back to the normal background alpha rather than zero.
            bgAlpha = mix(
