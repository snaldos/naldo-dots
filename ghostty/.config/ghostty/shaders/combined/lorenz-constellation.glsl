...
const float LORENZ_CULL_RADIUS = 1.72;
...
#define LC_ENABLE_RESONANCE_LINK 1
...
void renderLorenzBackground(out vec4 fragColor, vec2 fragCoord) {
    ...
    for (int objectIndex = 0; objectIndex < LORENZ_OBJECT_COUNT; objectIndex++) {
        ...
        if (
            abs(point.x - center.x) > sizeValue * LORENZ_CULL_RADIUS
            || abs(point.y - center.y) > sizeValue * LORENZ_CULL_RADIUS
        ) continue;
...
void applyLorenzCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    vec2 primaryPixels = lorenzUv(iTime, 0.0) * resolution;
    ...
    if (nearLink) {
        ...
        float dash = 0.64 + 0.36 * sin(
            linkAlong * LC_LINK_DASH_COUNT - iTime * LC_LINK_DASH_SPEED
        );
        float core = exp(-linkDistance / max(cursorSize * LC_LINK_WIDTH, 0.0002));
        float glow = exp(-linkDistance / max(cursorSize * LC_LINK_GLOW_WIDTH, 0.0005));
        effectLight += mix(LORENZ_ROSE, LORENZ_CYAN, linkAlong) * dash * (
            core * LC_LINK_CORE_STRENGTH + glow * LC_LINK_GLOW_STRENGTH
        );
