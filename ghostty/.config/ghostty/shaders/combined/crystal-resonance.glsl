...
#define CR_ENABLE_RESONANCE_LINK 1
...
void applyCrystalResonanceCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    vec2 mainObjectPixels = petUv(iTime, 0.0) * resolution;
    ...
    vec2 mainObject = crScenePoint(mainObjectPixels);
    ...
    if (nearLink) {
        float linkDistance = crSegmentDistance(point, head, mainObject);
        float linkAlong = crSegmentParameter(point, head, mainObject);
        float dash = 0.62 + 0.38 * sin(
            linkAlong * CR_LINK_DASH_COUNT - iTime * CR_LINK_DASH_SPEED
        );
        float linkCore = exp(-linkDistance / max(cursorSize * CR_LINK_WIDTH, 0.0002));
        float linkGlow = exp(-linkDistance / max(cursorSize * CR_LINK_GLOW_WIDTH, 0.0005));
        vec3 linkColor = mix(CR_ROSE, CR_CYAN, linkAlong);
        scene.rgb += linkColor * dash * life * contentMask * (
            linkCore * CR_LINK_CORE_STRENGTH
            + linkGlow * CR_LINK_GLOW_STRENGTH
        );
        float endpoint = gaussianPoint(point - mainObject, cursorSize * 0.76);
        scene.rgb += CR_WHITE * endpoint * life * CR_LINK_ENDPOINT_GLOW * contentMask;
    }
#endif
