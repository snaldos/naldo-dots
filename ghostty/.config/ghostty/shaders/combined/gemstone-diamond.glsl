...
#define GD_ENABLE_RESONANCE_LINK 1
...
void applyGemstoneDiamondCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    vec2 mainObjectPixels = gemUv(iTime, 0.0) * resolution;
    ...
    vec2 mainObject = gdScenePoint(mainObjectPixels);
    ...
    if (nearLink) {
        ...
        float dash = 0.62 + 0.38 * sin(
            linkAlong * GD_LINK_DASH_COUNT - iTime * GD_LINK_DASH_SPEED
        );
        float linkCore = exp(-linkDistance / max(cursorSize * GD_LINK_WIDTH, 0.0002));
        float linkGlow = exp(-linkDistance / max(cursorSize * GD_LINK_GLOW_WIDTH, 0.0005));
        vec3 linkColor = mix(GD_ROSE, GD_CYAN, linkAlong);
        scene.rgb += linkColor * dash * life * contentMask * (
            linkCore * GD_LINK_CORE_STRENGTH
            + linkGlow * GD_LINK_GLOW_STRENGTH
        );
        float endpoint = gaussianPoint(point - mainObject, cursorSize * 0.72);
        scene.rgb += GD_CYAN * endpoint * life * GD_LINK_ENDPOINT_GLOW * contentMask;
    }
#endif
