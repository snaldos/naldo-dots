...
const float KNOT_CULL_RADIUS = 2.10;
...
#define KC_ENABLE_RESONANCE_LINK 1
...
void renderKnotBackground(out vec4 fragColor, vec2 fragCoord) {
    ...
    for (int objectIndex = 0; objectIndex < KNOT_OBJECT_COUNT; objectIndex++) {
        ...
        if (
            abs(point.x - center.x) > sizeValue * KNOT_CULL_RADIUS
            || abs(point.y - center.y) > sizeValue * KNOT_CULL_RADIUS
        ) continue;
...
void applyKnotCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    vec2 primaryPixels = knotUv(iTime, 0.0) * resolution;
    ...
    if (nearLink) {
        ...
        float dash = 0.64 + 0.36 * sin(
            linkAlong * KC_LINK_DASH_COUNT - iTime * KC_LINK_DASH_SPEED
        );
        float core = exp(-linkDistance / max(cursorSize * KC_LINK_WIDTH, 0.0002));
        float glow = exp(-linkDistance / max(cursorSize * KC_LINK_GLOW_WIDTH, 0.0005));
        effectLight += mix(KNOT_ROSE, KNOT_CYAN, linkAlong) * dash * (
            core * KC_LINK_CORE_STRENGTH + glow * KC_LINK_GLOW_STRENGTH
        );
