...
const float MOBIUS_CULL_RADIUS = 2.15;
...
#define MC_ENABLE_RESONANCE_LINK 1
...
void renderMobiusBackground(out vec4 fragColor, vec2 fragCoord) {
    ...
    for (int objectIndex = 0; objectIndex < MOBIUS_OBJECT_COUNT; objectIndex++) {
        ...
        if (
            abs(point.x - center.x) > sizeValue * MOBIUS_CULL_RADIUS
            || abs(point.y - center.y) > sizeValue * MOBIUS_CULL_RADIUS
        ) continue;
        ...
        composite = mix(
            composite,
            MOBIUS_VOID,
            coreGlow * MOBIUS_CORE_DARKEN * backgroundMask
        );
...
void applyMobiusCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    vec2 primaryPixels = mobiusUv(iTime, 0.0) * resolution;
    ...
    if (nearLink) {
        ...
        float dash = 0.64 + 0.36 * sin(
            linkAlong * MC_LINK_DASH_COUNT - iTime * MC_LINK_DASH_SPEED
        );
        float core = exp(-linkDistance / max(cursorSize * MC_LINK_WIDTH, 0.0002));
        float glow = exp(-linkDistance / max(cursorSize * MC_LINK_GLOW_WIDTH, 0.0005));
        effectLight += mix(MOBIUS_GOLD, MOBIUS_CYAN, linkAlong) * dash * (
            core * MC_LINK_CORE_STRENGTH + glow * MC_LINK_GLOW_STRENGTH
        );
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    renderMobiusBackground(fragColor, fragCoord);
