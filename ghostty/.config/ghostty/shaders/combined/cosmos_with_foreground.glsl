...
#define BLACK_HOLE_LENS_MIX 0.00
...
float terminalContentMask(vec4 terminalColor) {
    // background-opacity-cells keeps Neovim backgrounds at the configured
    // Ghostty opacity, while antialiased foreground glyphs rise toward alpha
    // 1. Detect that alpha excess instead of comparing RGB values: applications
    // are free to use a background color different from iBackgroundColor.
    return smoothstep(
...
vec4 protectTerminalReadability(
    ...
    // Restore glyph pixels last so the galaxy, black-hole disk, meteors, and
    // cursor glow cannot recolor terminal content.
...
vec4 composeCosmicScene(
    ...
    vec3 finalColor = baseColor + meteorColor * backgroundMask;
...
const float CONTENT_PROTECTION = 0.00;
