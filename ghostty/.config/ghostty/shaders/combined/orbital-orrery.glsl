...
#define OC_ENABLE_RESONANCE_LINK 1
...
void applyOrbitalOrreryCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    vec2 mainObjectPixels = orbUv(iTime, 0.0) * resolution;
    ...
    vec2 mainObject = ocScenePoint(mainObjectPixels);
    ...
    if (nearLink) {
        ...
        float dash = 0.64 + 0.36 * sin(
            linkAlong * OC_LINK_DASH_COUNT - iTime * OC_LINK_DASH_SPEED
        );
        float linkCore = exp(-linkDistance / max(cursorSize * OC_LINK_WIDTH, 0.0002));
        float linkGlow = exp(-linkDistance / max(cursorSize * OC_LINK_GLOW_WIDTH, 0.0005));
        vec3 linkColor = mix(OC_GOLD, OC_CYAN, linkAlong);
        scene.rgb += linkColor * dash * life * contentMask * (
            linkCore * OC_LINK_CORE_STRENGTH
            + linkGlow * OC_LINK_GLOW_STRENGTH
        );
        float endpoint = gaussianPoint(point - mainObject, cursorSize * 0.74);
        scene.rgb += OC_CYAN * endpoint * life * OC_LINK_ENDPOINT_GLOW * contentMask;
    }
#endif
