// BUILD: standalone sierpinski-tetra-garden cursor
// Derived from combined/sierpinski-tetra-garden.glsl; mainImage never renders its background.
...
#define FC_ENABLE_RESONANCE_LINK 0    // standalone cursor: no links to invisible objects
#define FC_LINK_ALL_OBJECTS 1         // 1: every object; 0: primary object only
...
void applyFractalCursor(inout vec4 scene, vec2 fragCoord) {
    ...
    for (int linkIndex = 0; linkIndex < FT_OBJECT_COUNT; linkIndex++) {
        if (FC_LINK_ALL_OBJECTS == 0 && linkIndex > 0) continue;
        float linkIdentity = float(linkIndex);
        vec2 linkObjectPixels = fractalUv(iTime, linkIdentity) * resolution;
    ...
    for (int linkIndex = 0; linkIndex < FT_OBJECT_COUNT; linkIndex++) {
        ...
        float linkStrength = pow(FC_LINK_SECONDARY_FALLOFF, linkIdentity);
