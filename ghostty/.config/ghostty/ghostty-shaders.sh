...
#   GHOSTTY_PROC_ROOT   Process tree used by `reload` (defaults to /proc).
#
# =============================================================================
...
NO_RELOAD=0
GHOSTTY_PROC_ROOT="${GHOSTTY_PROC_ROOT:-/proc}"
...
Shader modes:
  ...
  - Combined: one shader supplies both stages. Enabling it disables both
    separate stages.
...
load_profile_id() {
  case "$GPU_PROFILE" in
  eco) GPU_PROFILE_ID=0 ;;
  balanced) GPU_PROFILE_ID=1 ;;
  quality) GPU_PROFILE_ID=2 ;;
  ultra) GPU_PROFILE_ID=3 ;;
  esac
}
...
profile_summary() {
  printf 'GPU profile: %s (%s)' "$(profile_display_name)" "$GPU_PROFILE"
}
...
reload_ghostty() {
  ...
  for proc in "$GHOSTTY_PROC_ROOT"/[0-9]*; do
