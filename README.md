...
`kdewallet`, and OpenSSH agent; one Plasma-only autostart invokes `ssh-add` and
KSSHAskPass without replacing Fedora's agent selection. There is no user portal
override and no desktop-removal phase. The acceptance and recovery boundaries
are in
