...
- operating-system or desktop package install, remove, or upgrade transactions; project-local dependency changes stay silent
...
Approval and authentication remain separate. Agent tool calls must spell elevation as `/usr/bin/sudo -n` or `/usr/bin/sudoedit -n`; the absolute path prevents PATH/function substitution and `-n` guarantees that the later tool process cannot read a password. After approval, the extension checks `/usr/bin/sudo -n -v`. If authentication is needed, interactive Pi suspends its TUI and launches the fixed `/usr/bin/sudo -v` binary with inherited terminal I/O. The user types into sudo's native hidden prompt; Pi then resumes and revalidates the credential.

Pi's built-in Bash worker starts in a detached terminal session, while sudo's default timestamp is scoped to the authenticating terminal and session. The extension therefore runs only the exact approved sudo command through a Bash backend backed by `pi.exec()`, which stays in Pi's terminal session and can reuse the native credential; all ordinary Bash calls remain on Pi's built-in backend. The approved command is matched again at execution and the ticket-aware route is single-use. Cancellation, failed authentication, a changed command, a non-reusable timestamp policy, or a required prompt outside TUI mode blocks the command.
