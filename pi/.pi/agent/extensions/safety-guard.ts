...
export function registerSafetyGuard(
  ...
  const setEnabled = (next: boolean, ctx: ExtensionContext, notify = true): void => {
    ...
    ctx.ui.notify(
      next
        ...
        : "YOLO mode ON · high-impact confirmations are bypassed; trusted sudo authentication remains active",
      next ? "info" : "warning",
    );
  };
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    // Trusted sudo transport is independent of the high-impact confirmation
    // policy. Even YOLO mode must never expose a password to a tool process.
