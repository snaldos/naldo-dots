...
async function integrationCheck(pi: ExtensionAPI): Promise<Check> {
  const [fish, ghostty, hx, herdr] = await Promise.all([
    ...
    commandResult(pi, "hx", ["--health", "typst"], 20_000),
  ...
  return { status: "pass", label: "Integrations", detail: "Fish syntax · Ghostty config · Helix Typst health · current Herdr Pi integration" };
}
