...
describe("safety gate mode", () => {
  ...
  it("fails closed without a confirmation UI unless YOLO is active", async () => {
    ...
    const denied = await guarded.emit("tool_call", {
      ...
      input: { command: "dnf remove systemd" },
    }) as { block?: boolean };
    assert.equal(denied.block, true);
...
describe("sudo authentication", () => {
  ...
  it("keeps trusted sudo authentication active while YOLO bypasses confirmation", async () => {
    ...
    assert.equal(await app.emit("tool_call", sudoCall("sudo-yolo-auth")), undefined);
    assert.equal(app.confirmationCount(), 0);
    assert.equal(app.sudoValidationCount(), 2);
    assert.equal(app.sudoAuthenticationCount(), 1);
  });

  it("keeps unsafe sudo authentication paths blocked in YOLO mode", async () => {
    const app = harness(true, true, { validationResults: [true] });
    await app.emit("session_start", {});

    const result = await app.emit("tool_call", sudoCall("sudo-yolo-unsafe", "sudo -n true")) as {
      block?: boolean;
      reason?: string;
    };
    ...
    assert.equal(app.confirmationCount(), 0);
    assert.equal(app.sudoValidationCount(), 0);
    assert.equal(app.sudoAuthenticationCount(), 0);
  });
