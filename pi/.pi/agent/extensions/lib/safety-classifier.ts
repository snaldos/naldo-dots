...
function classifySystemOperation(executable: string, args: string[]): SafetyIssue[] {
  ...
  if (["dracut", "grub-install", "grub-mkconfig", "kernel-install"].includes(executable)) {
  ...
  if (["pkill", "killall"].includes(executable)) {
    ...
    if (/(?:niri|noctalia|gdm|systemd|wayland|ghostty|herdr|sshd|tailscaled)/i.test(target)) {
