...
The shared machine selector is not compositor-owned: read optional override
`~/.config/naldo/machine-profile/profile`, otherwise tracked `default`. Both
profiles use XKB `us`. Hyprland consumes the selected value directly from Lua;
the installer renders Niri's ignored `~/.config/niri/machine.kdl` to include
`profiles/desktop.kdl` or `profiles/laptop.kdl`.

The Hyprland theme launcher stores its machine-local Zen opacity/blur choice as
`opaque` or `transparent` under `$XDG_STATE_HOME/hypr/zen-browser-theme`;
`rules.lua` reads it on every config load. Niri instead includes ignored
`~/.config/niri/zen-browser-theme.kdl`, which its compositor-specific theme
launcher updates atomically and validates before reloading.
