...
# Keep the current palette source. Built-in and community palettes use
# different names for the corresponding theme families.
case "$source:$wall" in
  ...
  community:*/default/*)     palette="Noctalia legacy" ;;
...
if [[ "$source" != "wallpaper" && "$current_palette" != "$palette" ]]; then
