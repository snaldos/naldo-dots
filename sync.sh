...
if ! git -c core.whitespace=-blank-at-eol,-blank-at-eof,-space-before-tab \
  diff --cached --check; then
  fail "staged changes contain leftover conflict markers"
fi

if ! git diff --cached --check; then
  log "WARNING: staged whitespace errors shown above will be synced unchanged" >&2
fi
